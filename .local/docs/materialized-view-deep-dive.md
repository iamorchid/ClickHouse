...
### `SummingMergeTree` 和 `AggregatingMergeTree` 的区别

两者都可以在后台 merge 时把相同 `ORDER BY` key 的多行合并，但合并语义不同：

| 引擎 | 目标列保存什么 | 后台 merge 做什么 | 查询方式 | 适合场景 |
|---|---|---|---|---|
| `SummingMergeTree` | 普通数值列，例如 `UInt64`、`Float64` | 对非 key 数值列做加法 | 通常继续用 `sum` 和 `GROUP BY` | `pv`、`bytes`、`amount` 这类可加指标 |
| `AggregatingMergeTree` | 聚合状态列，例如 `AggregateFunction(uniq, UInt64)` | 按聚合函数语义合并状态 | 用 `uniqMerge`、`avgMerge`、`sumMerge` 等 `-Merge` 函数 | `uniq`、`avg`、`quantile` 这类不能简单相加的聚合 |

关键区别是：`SummingMergeTree` 不知道列背后的聚合语义，只会把普通数值列相加；`AggregatingMergeTree` 保存的是聚合函数的中间状态，后台 merge 和查询时都按对应聚合函数的语义合并状态。

例如 `sum(amount)` 可以用 `SummingMergeTree` 保存普通 `amount` 数值，也可以用 `AggregatingMergeTree` 保存 `sumState(amount)`。但 `uniq(user_id)` 不能用普通数值列保存每个 block 的 `uniq` 结果后再相加，因为不同 block 之间可能有重复用户；它需要保存 `uniqState(user_id)`，最后用 `uniqMerge` 合并状态。
...
uniqState({2,3}) + sumState(70)
        ...
        ▼
query:
uniqMerge 合并状态 {1,2,3} -> 3
sumMerge  合并状态 150 + 70 -> 220
```
...
`avg` 的关键点是：平均值不能直接按平均值继续求平均。比如第一批有 2 条数据，平均值是 75；第二批有 1 条数据，平均值是 30。最终平均值不是 `(75 + 30) / 2 = 52.5`，而是 `(100 + 50 + 30) / 3 = 60`。

所以保存 `avg` 物化结果时，不应该把目标列设计成普通数值列并写入 `avg(amount)`。正确方式是使用 `AggregatingMergeTree` 保存 `avg` 的聚合状态：

```sql
CREATE TABLE sales_daily_avg
(
    day Date,
    avg_amount AggregateFunction(avg, UInt64)
)
ENGINE = AggregatingMergeTree
ORDER BY day;

CREATE MATERIALIZED VIEW sales_daily_avg_mv TO sales_daily_avg AS
SELECT
    toDate(ts) AS day,
...
```text
2026-06-15 amount = 100
2026-06-15 amount = 50
```

物化视图只处理本次插入 block，并写入一条状态：

```text
day = 2026-06-15
avg_amount = avgState(sum = 150, count = 2)
```

源表再写入第二批明细：

```text
2026-06-15 amount = 30
```

目标表不会回头扫描历史明细，而是追加新的状态：

```text
day = 2026-06-15
avg_amount = avgState(sum = 30, count = 1)
```

查询时用 `avgMerge` 合并这些状态：

```sql
SELECT
    day,
    avgMerge(avg_amount) AS avg_amount
FROM sales_daily_avg
GROUP BY day
ORDER BY day;
```
...
```text
avgState(sum = 150, count = 2)
        │
        ├─ avgMerge -> sum = 180, count = 3 -> avg = 60
        │
avgState(sum = 30, count = 1)
```

查询加速来自两层：

1. 写入时已经按 `day` 做了局部聚合，目标表行数从明细行缩小为按天的状态行。
2. 查询时只合并 `avg` 状态，不再读取 `events` 明细，也不再对每条明细重新执行 `toDate` 和 `GROUP BY`。

后台 merge 发生时，`AggregatingMergeTree` 可以把相同 `ORDER BY` key 的状态合并成更少的状态行：

```text
merge 前:
2026-06-15 avgState(sum = 150, count = 2)
2026-06-15 avgState(sum = 30, count = 1)

merge 后:
2026-06-15 avgState(sum = 180, count = 3)
```

但查询仍然应该使用 `GROUP BY day` 和 `avgMerge(avg_amount)`。原因和前面一致：后台 merge 是异步的，查询执行时目标表里可能同时存在多个 part 和多条同 key 状态。
...
4. 对复杂聚合使用 `AggregatingMergeTree`，写入 `sumState`、`uniqState`、`avgState`，查询 `sumMerge`、`uniqMerge`、`avgMerge`。
...
它的查询加速来自预计算和更适合查询的目标表布局。新写入数据不会和历史明细表重新全量计算，而是生成增量结果追加到目标表。后续的语义合并由目标表引擎和查询共同完成：`MergeTree` 需要查询时聚合，`SummingMergeTree` 可以后台求和但查询仍建议聚合，`AggregatingMergeTree` 保存聚合状态并通过 `-Merge` 聚合函数得到最终结果，例如用 `avgState` 保存 `avg` 的中间状态，再用 `avgMerge` 合并状态得到最终平均值。
